...
That whole tree is not built in. It exists because one entry in
[`../Visualization.yaml`](../Visualization.yaml) asks for it — a name, a destination folder, and
optional arguments:
...
`dest` is the folder, not a file: the `DashBoard` visualization writes every page above beneath it,
and its args are what bound how far the tree reaches in each direction.

| To… | Do this in `Visualization.yaml` |
| --- | --- |
| Stop seeing this tree | Delete the entry, or set `enabled: false` on it |
| Move the whole tree | Change `dest` — the layout of the vault is yours |
| Render more past months | Raise `args.prev-months` (this vault: 3) |
| Change the forecast horizon | Change `args.future-months` (this vault: 8) |
| Preview a change first | `./wraith render DashBoard --future-months 12` |

Nothing here is rendered that you did not ask for, and a tree you remove stops being refreshed on
the next tick — the files already written stay where they are until you delete them. The catalog of
every available visualization, the tree it writes and its arguments is in
[`../Help/Visualization.md`](../Help/Visualization.md).
...
> Generated by `./wraith tick` from the tasks in [`../Help/Task.md`](../Help/Task.md), laid out by
