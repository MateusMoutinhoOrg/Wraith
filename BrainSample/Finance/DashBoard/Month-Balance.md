# 🧾 Month Balance — August / 2026

> **Period:** 01-aug-2026 → 31-aug-2026 · **Partial close:** 16-aug-2026 (52% of the month)
> **Currency:** BRL (R$) · **Scope:** Individual + MEI (Wraith Software)
> **Status:** 🔴 **27 entries, result -R$ 200** — `Vices` moves almost every day and drives the leak.

---

## 1. 📌 Headline Numbers

| Line                       | Value          | Movements | Status |
| -------------------------- | -------------: | --------: | ------ |
| **(+) Total received**     |       R$ 500   |         1 | 🔴     |
| **(−) Total spent**        |       R$ 730   |        26 | 🟡     |
| **(+) Refunds / returns**  |        R$ 30   |         1 | ⚪     |
| **(=) Month result**       |  **-R$ 200**   |        27 | 🔴     |
| Opening cash (01-aug)      |     R$ 1,300   |         — | 🟡     |
| **Closing cash (day 16)**  | **R$ 1,100**   |         — | 🟡     |

> ℹ️ *Total spent* is gross. The `Poker` refund of R$ 30 is a bidirectional inflow on a
> discretionary tag, so net expense = R$ 730 − R$ 30 = **R$ 700**, the figure carried into
> [`Month-Results.md`](Month-Results.md).

---

## 2. 📒 Statement — Realized (01-aug → 16-aug)

| Date        | Transaction               | Tag         |       Value |  Balance |
| ----------- | ------------------------- | ----------- | ----------: | -------: |
| 01-aug-2026 | Rent + utilities          | `Home`      |     -R$ 300 | R$ 1,000 |
| 02-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 |   R$ 990 |
| 02-aug-2026 | Supermarket — weekly shop | `Food`      |      -R$ 45 |   R$ 945 |
| 03-aug-2026 | Fuel                      | `Transport` |      -R$ 15 |   R$ 930 |
| 04-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 |   R$ 920 |
| 04-aug-2026 | Bakery                    | `Food`      |      -R$ 18 |   R$ 902 |
| 05-aug-2026 | Cloud hosting — Wraith    | `Business`  |      -R$ 20 |   R$ 882 |
| 06-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 |   R$ 872 |
| 06-aug-2026 | Poker night — buy-in      | `Poker`     |      -R$ 50 |   R$ 822 |
| 07-aug-2026 | Supermarket               | `Food`      |      -R$ 32 |   R$ 790 |
| 08-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 |   R$ 780 |
| 08-aug-2026 | Ride app                  | `Transport` |      -R$ 12 |   R$ 768 |
| 09-aug-2026 | Poker night — cash out    | `Poker`     |      +R$ 30 |   R$ 798 |
| 10-aug-2026 | Buy Cigarettes            | `Vices`     |      -R$ 10 |   R$ 788 |
| 10-aug-2026 | Lunch out                 | `Food`      |      -R$ 25 |   R$ 763 |
| 11-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 |   R$ 753 |
| 11-aug-2026 | **Client invoice — MEI**  | `Freelance` | **+R$ 500** | R$ 1,253 |
| 12-aug-2026 | Supermarket               | `Food`      |      -R$ 28 | R$ 1,225 |
| 12-aug-2026 | Ride app                  | `Transport` |      -R$ 10 | R$ 1,215 |
| 13-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 | R$ 1,205 |
| 13-aug-2026 | Domain renewal            | `Business`  |      -R$ 20 | R$ 1,185 |
| 14-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 | R$ 1,175 |
| 14-aug-2026 | Poker night — buy-in      | `Poker`     |      -R$ 25 | R$ 1,150 |
| 15-aug-2026 | Supermarket               | `Food`      |      -R$ 22 | R$ 1,128 |
| 15-aug-2026 | Fuel                      | `Transport` |       -R$ 8 | R$ 1,120 |
| 16-aug-2026 | Cigarettes                | `Vices`     |      -R$ 10 | R$ 1,110 |
| 16-aug-2026 | Snack                     | `Food`      |      -R$ 10 | R$ 1,100 |

---

## 3. 📅 Scheduled — Not Yet Realized (17-aug → 31-aug)

| Date        | Transaction                    | Tag         |        Value | Confidence |
| ----------- | ------------------------------ | ----------- | -----------: | ---------- |
| 25-aug-2026 | **Client receivable — MEI**    | `Freelance` | +R$ 1,500\*  | 🟡 Medium  |
| 17→31-aug   | Food at current pace           | `Food`      |   -R$ 150\*  | 🟢 High    |
| 17→31-aug   | Cigarettes at current pace     | `Vices`     |    -R$ 70\*  | 🔴 High    |
| 17→31-aug   | Transport at current pace      | `Transport` |    -R$ 40\*  | 🟢 High    |
| 28-aug-2026 | DAS — MEI                      | `Business`  |    -R$ 20\*  | 🟢 Certain |
| 30-aug-2026 | Card interest                  | `Debt`      |    -R$ 20\*  | 🟡 Medium  |
| **Total**   |                                |             | **+R$ 1,200\*** | —       |

`*` projected value, not realized. Projected month close: **+R$ 1,000\***.

---

## 4. 🧮 Totals by Tag (day 16)

| Tag         | Mov. |  Inflows |  Outflows |      Net | % of expense |
| ----------- | ---: | -------: | --------: | -------: | -----------: |
| `Freelance` |    1 |  R$ 500  |     R$ 0  | +R$ 500  |           —  |
| `Home`      |    1 |    R$ 0  |   R$ 300  | -R$ 300  |       42.9%  |
| `Food`      |    7 |    R$ 0  |   R$ 180  | -R$ 180  |       25.7%  |
| `Vices`     |    9 |    R$ 0  |    R$ 90  |  -R$ 90  |       12.9%  |
| `Poker`     |    3 |   R$ 30  |    R$ 75  |  -R$ 45  |       10.7%  |
| `Transport` |    4 |    R$ 0  |    R$ 45  |  -R$ 45  |        6.4%  |
| `Business`  |    2 |    R$ 0  |    R$ 40  |  -R$ 40  |        5.7%  |
| **Total**   | **27** | **R$ 530** | **R$ 730** | **-R$ 200** | **100%** |

```
Expense distribution
Home        ██████████████████████  42.9%
Food        █████████████░░░░░░░░░  25.7%
Vices       ██████▌░░░░░░░░░░░░░░░  12.9%
Poker       █████▌░░░░░░░░░░░░░░░░  10.7%
Transport   ███▎░░░░░░░░░░░░░░░░░░   6.4%
Business    ██▉░░░░░░░░░░░░░░░░░░░   5.7%
```

---

## 5. 📈 Balance Curve (day 16)

```
R$ 1,300 ┤█
R$ 1,200 ┤                          ███████████████
R$ 1,100 ┤                          ███████████████
R$ 1,000 ┤ ██
R$   900 ┤ ██████
R$   800 ┤ ████████████
R$   700 ┤            ██
         └──┬──┬──┬──┬──┬──┬──┬──┬──
           01 03 05 07 09 11 13 15
```

> 📉 **Low point:** R$ 753 on 11-aug, hours before the client invoice cleared.

---

## 6. 🚩 Reading the Statement

| Finding                                                        | Impact             | Status |
| -------------------------------------------------------------- | ------------------ | ------ |
| `Vices` posted on **9 of 16 days** at R$ 10 each                | R$ 3,650/year pace | 🔴     |
| `Food` fragmented into 7 tickets averaging R$ 25.7              | No shopping list   | 🟡     |
| Single income event — 100% of cash depends on one client        | Concentration risk | 🔴     |
| `Study` and `Debt` idle all month                               | Envelopes unused   | ⚪     |
| Balance never went negative                                     | No overdraft fee   | 🟢     |

---

## 7. 🔗 Where This Feeds

| Consumer                                       | Uses this file for                        |
| ---------------------------------------------- | ----------------------------------------- |
| [`Month-Results.md`](Month-Results.md)         | Raw totals for actual vs. projected       |
| [`Budget.md`](Budget.md)                       | Consumption against the envelope caps     |
| [`Cash-Flow.md`](Cash-Flow.md)                 | Daily balance and the projected low point |
| [`Categories-Tags.md`](Categories-Tags.md)     | Performance and frequency per tag         |

---

## 📖 Legend

| Symbol | Meaning                                        |
| ------ | ---------------------------------------------- |
| 🟢     | On target / healthy                            |
| 🟡     | Attention — between 70% and 100% of the limit  |
| 🔴     | Off target / requires immediate action         |
| ⚪     | Inactive / no movement                         |
| `*`    | Projected value, not realized                  |

> 📌 *Fictional document, used as a model for personal + small-business financial management.*
