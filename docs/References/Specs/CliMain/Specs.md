...
- It imports the library under the `wraith`-prefixed aliases every outside consumer uses: `wraithadapter` for `adapters/<name>`, `wraithlib` for `sandbox`, `wraithtypes` for `sandbox/contracts/api`.
...
2. **Imports**: `os` and whatever resolving the data directory needs, plus `wraithadapter "…/adapters/standard"` and `wraithlib "…/sandbox"`.
3. **Configuration constants**: the data directory's name and the environment variable overriding it.
4. **`main` function**: build deps through the adapter, inject with `wraithlib.New`, and `os.Exit(l.Sandboxmain(os.Args[1:]))`.
