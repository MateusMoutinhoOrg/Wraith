...
4. **Scratch setup**: a `mktemp -d` work directory with a cleanup `trap`, the `go build` of `./cmd/main` into it, a `cd` into a vault created inside it, and a `wraith()` shell function calling the built binary.
