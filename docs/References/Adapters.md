...
|---------|---------|----------|----------|
| `standard` | [standard.New](/docs/References/PublicApi/standard.New.md) | Real wall clock and real `time.Sleep`; `Printf` to standard output; embedded Verb parser over `os.Args[1:]`; embedded Keep database on the filesystem, one file per key under a caller-chosen base path; the project's assets compiled into the binary and served whole; the filesystem over `os`/`path/filepath`; HTTP over `net/http` with a bounded round trip | You want the default: a vault on disk, surviving across runs |

An adapter filling `Printf` with a buffer and `VerbLib` with a fixed argument vector is what makes the command-line interface itself — `api.Lib.Sandboxmain` — runnable without a terminal. Filling `Sleep` with a no-op is what makes a hundred ticks of `watch` cost no time at all.
...
One field is declared and filled like any other dependency, but this brain never calls it: [`NewRequest`](/docs/References/PublicApi/requestdeps.Request.md). It ships because this repository is a template — a derived brain whose task pulls a bank statement speaks HTTP without designing a contract for it first.
