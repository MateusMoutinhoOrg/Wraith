...
### Rules
- A public function is only usable once its factory's return value is assigned in the library's `New(d deps.Deps, databasePath string)` constructor inside [sandbox/lib/new.go](/sandbox/lib/new.go).
...
1. Declare the function as a field of the `Lib` struct in [sandbox/contracts/api/api.go](/sandbox/contracts/api/api.go):
   ...
   type Lib struct {
       Deps      deps.Deps
       // ... existing functions
       HasTask   func(name string) bool // new function
   }
   ```
2. Write its factory in a new or existing file under [sandbox/lib/publicfunctions/](/sandbox/lib/publicfunctions/). The factory must return the closure:
   ...
   // HasTaskFactory returns the closure that fills api.Lib.HasTask.
   func HasTaskFactory(l *api.Lib) func(name string) bool {
...
3. Assign the factory's return value in the package's `New` constructor inside [sandbox/lib/new.go](/sandbox/lib/new.go) — without this line the field stays nil and panics when called:
   ...
   func New(d deps.Deps, databasePath string) api.Lib {
       ...
       l.HasTask = publicfunctions.HasTaskFactory(&l) // register the new function
       return l
   }
   ```
...
6. Expose it in the public API:
   ...
   - Create its detail page under [docs/References/PublicApi/](/docs/References/PublicApi/), named `api.HasTask.md`, following [HandleDocuments.md](/docs/Tutorials/HandleDocuments.md).
