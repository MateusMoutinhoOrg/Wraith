...
2. Change the module path on the first line (e.g., `module github.com/MateusMoutinhoOrg/Wraith`) to the new module path.
3. Replace every internal import path. Use the IDE's global "Find and Replace", or run:
   - **macOS:**
     ```sh
     find . -type f \( -name '*.go' -o -name '*.md' \) -exec sed -i '' 's|github.com/MateusMoutinhoOrg/Wraith|<your-new-module-path>|g' {} +
     ```
   - **Linux:**
     ```sh
     find . -type f \( -name '*.go' -o -name '*.md' \) -exec sed -i 's|github.com/MateusMoutinhoOrg/Wraith|<your-new-module-path>|g' {} +
