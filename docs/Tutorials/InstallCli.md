...
1. Copy and paste the command below corresponding to your Mac's processor into your terminal to download and install the binary:
   ...
   **Apple Silicon (M1/M2/M3 / arm64)**
   ```bash
   curl -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/macarm64.bin -o agnos-cli && \
     chmod +x agnos-cli && \
   ...
   **Intel (x86_64)**
   ```bash
   curl -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/mac86.bin -o agnos-cli && \
     chmod +x agnos-cli && \
     sudo mv agnos-cli /usr/local/bin/ && \
     agnos-cli version
   ```
...
1. Copy and paste the command below corresponding to your architecture into your terminal to download and install the binary:
   ...
   **x86_64 (64-bit)**
   ```bash
   curl -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/linux86.out -o agnos-cli && \
     chmod +x agnos-cli && \
     sudo mv agnos-cli /usr/local/bin/ && \
     agnos-cli version
   ```
   ...
   **ARM64**
   ```bash
   curl -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/linuxarm64.out -o agnos-cli && \
     chmod +x agnos-cli && \
     sudo mv agnos-cli /usr/local/bin/ && \
     agnos-cli version
   ```
   ...
   **x86 (32-bit)**
   ```bash
   curl -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/linuxi32.out -o agnos-cli && \
     chmod +x agnos-cli && \
     sudo mv agnos-cli /usr/local/bin/ && \
     agnos-cli version
   ```
...
1. Open PowerShell and copy and paste the command below corresponding to your architecture. It will download the binary, install it to your user profile, and update your `PATH`:
   ...
   **x86_64 (64-bit)**
   ...
   $dir = "$HOME\.local\bin"; `
     ...
     curl.exe -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/windows86.exe -o "$dir\agnos-cli.exe"; `
   ...
   **x86 (32-bit)**
   ...
   $dir = "$HOME\.local\bin"; `
     New-Item -ItemType Directory -Force -Path $dir | Out-Null; `
     curl.exe -sL https://github.com/MateusMoutinhoOrg/Wraith/releases/latest/download/windowsi32.exe -o "$dir\agnos-cli.exe"; `
     if ($env:PATH -notlike "*$dir*") { `
       [Environment]::SetEnvironmentVariable('PATH', [Environment]::GetEnvironmentVariable('PATH', 'User') + ";$dir", 'User'); `
       $env:PATH += ";$dir"; `
       Write-Host "Added $dir to your PATH (restart the terminal for full effect)"; `
